...
class WorkflowContractTests(RepoTestCase):
    ...
    def test_documentation_recheck_contract_is_documented(self) -> None:
        ...
        implement_task_content = (REPO_ROOT / "docs" / "policy" / "22-implement-task.md").read_text(encoding="utf-8")
        helper_protocol_content = (REPO_ROOT / "docs" / "policy" / "23-helper-protocol.md").read_text(encoding="utf-8")
        skill_content = (REPO_ROOT / "skills" / "implement-task" / "SKILL.md").read_text(encoding="utf-8")
        ...
        self.assertIn("구현 단계에서 focused validation 전에 함께 반영", implement_task_content)
        self.assertIn("sync_instructions.py", implement_task_content)
        self.assertIn("문서 diff도 slice budget에 포함", implement_task_content)
        self.assertIn("workflow.toml [slice_budget]", implement_task_content)
        self.assertIn("longer wait -> optional queued status probe -> background or natural completion", helper_protocol_content)
        self.assertIn("Immediate status check requires explicit cancel path", helper_protocol_content)
