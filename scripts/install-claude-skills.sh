...
usage() {
  cat <<'USAGE'
Usage: scripts/install-claude-skills.sh [options]

Install this repository's skills into Claude Code skills directory.

Options:
  --copy         Copy skill folders instead of symlinking
  --link         Symlink skill folders (default)
  --dest <path>  Override destination directory (default: $CLAUDE_HOME/skills)
