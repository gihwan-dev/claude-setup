...
토론을 시작하려면 다음 중 하나가 필수다:
- **텍스트 설명**: 2문장 이상의 구체적 계획 또는 설계 내용
- **파일 경로**: 설계 문서, 계획 파일, 또는 코드 파일의 경로
- **컨텍스트 요약**: 현재 작업 중인 설계의 핵심 결정사항

"토론해줘"만 입력된 경우, 반드시 "무엇에 대해 토론할까요? 설계 내용이나 파일 경로를 제공해주세요."로 물어본다.

## Success Criteria

토론은 다음 3가지 산출물이 모두 완성되어야 성공이다:
1. **합의사항 테이블** — 최소 1개 이상의 결정 항목 포함
2. **수정된 계획(Diff)** — 원본 대비 변경/유지 사항 명시
3. **품질 기준** — 검증 가능한 기준 1개 이상

3가지가 모두 채워지지 않으면 토론을 완료하지 않는다.
