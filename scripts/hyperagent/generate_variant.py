...
def meta_for_plan(plan: VariantPlan, created_at: str, score_source: str, llm_requested: bool) -> dict[str, Any]:
    ...
    return {
        ...
        "llm_status": "not_implemented" if llm_requested else "not_requested",
        "parent_variant": None,
...
def execute_plan(plan: VariantPlan, created_at: str, score_source: str, llm_requested: bool) -> dict[str, Any]:
    ...
    meta = meta_for_plan(plan, created_at, score_source, llm_requested)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate rule-based HyperAgent prompt variants from score reports.")
    ...
    parser.add_argument("--llm", action="store_true", help="Reserved for future LLM-based generation.")
...
def run(argv: list[str]) -> int:
    ...
    if args.llm:
        warn("--llm is accepted as a future interface; rule-based generation is used in this implementation")
    ...
    variants = (
        ...
        if args.dry_run
        else [execute_plan(plan, created_at, score_source, args.llm) for plan in plans]
    )
    output = {
        "schema_version": SCHEMA_VERSION,
