...
8. 검증이 통과하면 같은 `worker`가 commit-only로 재개하고 `STATUS.md`를 manager-facing 요약으로 갱신한다.
...
- writer가 editing 중이면 no review/diff inspection while writer is still editing 원칙을 유지한다.
- slice budget이 small slices 기준(`repo-tracked files 3개 이하`, 순 diff `150 LOC 내외`)을 넘기면 `split/replan before spawn`으로 되돌린다.
