...
- `delivery_strategy=ui-first`면 `UX_SPEC.md` 안에 `figma-less-ui-design`의 `UI Planning Packet`을 재사용한다. section order는 `Goal/Audience/Platform`, `Visual Direction + Anti-goals`, `Reference Pack (adopt/avoid)`, `Layout/App-shell Contract`, `Token + Primitive Contract`, `Screen/Flow/State Coverage`, `Review Loop`, `Implementation Prompt/Handoff`를 그대로 유지한다.
...
- no-Figma UI-first planning이거나 `UX_SPEC.md` packet이 약하면 `skills/figma-less-ui-design/SKILL.md`를 읽고 packet contract를 재사용한다.
...
13. `delivery_strategy=ui-first`면 `source_of_truth.ux = UX_SPEC.md`를 고정하고 `UX_SPEC.md`에 `UI Planning Packet`을 남긴다. `Layout/App-shell Contract`, `Token + Primitive Contract`, `Screen/Flow/State Coverage`, `Review Loop`가 정리되기 전에는 integration slice를 만들지 않는다.
...
`delivery_strategy=ui-first`면 `ux-journey-critic` fan-out이 기본값이자 mandatory다. `product-planner`, `web-researcher`, `solution-analyst`, `structure-planner`, `architecture-reviewer`는 goal/scope, external benchmark, option comparison, 구조 분해, public/shared boundary 리스크가 있을 때만 conditional로 추가한다.
...
- `delivery_strategy=ui-first`면 `UX_SPEC.md`가 `UI Planning Packet` exact order를 지키고, 기존 style source가 있을 때 `reuse + delta`를 기록했는가?
